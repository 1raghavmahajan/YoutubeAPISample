 ...
 *
 * The ActionBar is the only view allowed to overlay the player, so it is a useful place to put custom application controls when the video is in
 * fullscreen. The ActionBar can not change back and forth between normal mode and overlay mode, so to make sure our application's content is not
 * covered by the ActionBar we want to pad our root view when we are not in fullscreen.
 */
@TargetApi(11)
public class ActionBarDemoActivity extends YouTubeFailureRecoveryActivity implements
  ...
  /**
   * This is a FrameLayout which adds top-padding equal to the height of the ActionBar unless disabled by {@link #setEnablePadding(boolean)}.
   */
